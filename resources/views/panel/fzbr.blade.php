@php
    ...
    $title = $ui('fzbr_title', $locale === 'cg' ? 'Besplatne rezervacije' : 'Free reservations');
    ...
    $uploadHint = $ui('documents_hint', $locale === 'cg'
        ? "Priložite dokumentaciju kojom se potvrđuje angažovanje agencije/prevoznika (ugovor, račun, sporazum...).\n\nUkoliko trenutno ne raspolažete svim potrebnim dokumentima, možete poslati ono što imate, a preostalu dokumentaciju dostaviti naknadno."
        : "Attach documentation proving the engagement of the agency/carrier (contract, invoice, agreement, etc.) by liable parties exempt from payment for an organised visit to Kotor.\n\nIf you do not yet have all required documents, you may submit what you have now and provide the remainder later.");
@endphp

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ $p('nav_fzbr', $locale === 'cg' ? 'Besplatne rezervacije' : 'Free reservations') }}</h2>
    </x-slot>
