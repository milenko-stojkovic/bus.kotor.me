...
- 
- **PowerShell u Cursoru:** ne koristiti `&&` za lančanje komandi (stariji PS); koristiti `;` ili posebne linije. Iz korena repoa: `Set-L**`php` često nije u PATH-u** u Cursor terminalu. Za **`php artisan`** koristi **jedno od**:
  ...
  3. **Eksplicitna putanja** (primer; folder verzije proveri sa `dir C:\laragon\bin\php`):  
     `C:\laragon\bin\php\php-8.3.30-Win32-vs16-x64\php.exe artisan ...`ocation c:\laragon\www\bus.kotor.me; .\laragon-artisan.ps1 test` ili **`.\laragon-artisan.cmd test`**.
