...
- **2026-04-02** — **Mail/PDF hardening + fake fiskal realizam:** automatsko kreiranje `storage/app/invoices` za paid/free PDF generatore; `SendInvoiceEmailJob` anti-dup guard (DB lock + sending state) i ne šalje email bez PDF attachmenta; fake fiskal response vraća `Operator` i verifikacioni URL sa `ord`/`crtd` (Interni broj u PDF-u); usklađen `paid-invoice` tekst oslobođenja i QR blok (veći QR, bez praznih redova).
