...
- **`system_config_fake_production`** — samo u **`production`** (ili ručno `--assume-production` u testu): fake bank/fiskal (`BANK_DRIVER` / `FISCALIZATION_DRIVER`, uz legacy fallback `payment.provider`) ili `payment.fake_e2e_sync`.
