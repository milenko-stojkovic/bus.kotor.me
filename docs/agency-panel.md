...
- **Kartica — ponovni klik dok je pending:** isti booking (isti korisnik, datum, termini) reuse-uje sačuvani Bankart redirect (`payment_redirect_url`); v. **`workflow-placanje-temp-data.md`** §1d.
...
- Polja: **name**, **lang** (`cg` / `en`), **email**, **password** (trenutna / nova / potvrda) sa istim eye partialima kao auth; **`country`** — vidljivi **dropdown** (država platne kartice; v. **`auth-and-guests.md`** § Država platne kartice). Ako je u bazi sačuvana nevažeća vrednost (npr. legacy **`OTHER`**), prikazuje se upozorenje **`country_invalid_stored`** dok korisnik ne izabere validan ISO kod.
